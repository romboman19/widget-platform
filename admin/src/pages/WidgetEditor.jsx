    ...
    <>
      ...
      {!cfg.buttons?.length ? (
        <div className="bg-amber-50 rounded-lg p-4 border border-amber-200 mb-4">
          <p className="text-sm text-amber-800 font-medium mb-1">Старий формат FLOATING_MENU більше не редагується через v1 UI.</p>
          <p className="text-sm text-amber-700">Для цього віджета потрібно один раз мігрувати конфіг у формат кнопок v2, після чого редагування буде доступне нижче.</p>
          <button
            onClick={() => {
            ...
            className="mt-3 flex items-center gap-1 px-4 py-2 text-sm font-medium text-white bg-amber-600 hover:bg-amber-700 rounded-lg"
          >
            <Plus size={16} /> Мігрувати у кнопки v2
          </button>
        </div>
      ) : null}
