...
**Анімації появи:**
- `fade` — плавна поява
- `slide-up` — поява знизу вгору
- `slide-down` — поява зверху вниз
- `slide-left` — поява справа наліво
- `slide-right` — поява зліва направо
- `zoom` — поява через збільшення
- `bounce` — поява з підскоком
- `elastic` — пружна поява
- `flip` — поява з переворотом

**Анімації привернення уваги:**
- `pulse` — пульсація
- `shake` — тремтіння / струс
- `wobble` — хитання
- `spin` — обертання
- `swing` — маятниковий рух
- `bounce` — підстрибування
- `tada` — святкова акцентна анімація

### Робота в команді
- **Власник (Owner)** — керує всією платформою, створює сайти, додає користувачів, видає доступи та бачить код вставки
- **Редактор (Editor)** — працює лише з тими сайтами, до яких йому дали доступ
- **Доступи до конкретних сайтів** — можна одразу призначати під час створення сайту або редагувати пізніше на сторінці сайту
- **Безпечний розподіл ролей** — редактори можуть створювати й редагувати віджети, але не можуть змінювати системні налаштування сайту, створювати нові сайти чи керувати користувачами
- **Зручний сценарій для команди** — власник веде структуру, а дизайнер, маркетолог або контент-менеджер працює тільки у своїй зоні відповідальності
...
- ♿ **ARIA-позначки та ролі** — допоміжні технічні описи для екранних читачів
- ⌨️ **Керування з клавіатури** — можна користуватися елементами без мишки
- 🔍 **Підтримка програм озвучення екрана** — інтерфейс краще читається спеціальними засобами доступності
- 🎚️ **Підтримка зменшення анімацій** — для користувачів, яким некомфортно від активного руху на екрані
- 🔲 **Режим кращого контрасту** — інтерфейс легше сприймається в складних умовах або при порушеннях зору
